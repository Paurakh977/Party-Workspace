    ...
    const fetchPositions = async () => {
      try {
        const response = await axios.get<Position[]>(
          process.env.NEXT_PUBLIC_BE_HOST + "/positions",
        );
      ...
      <div className="p-7">
        <form onSubmit={handleSubmit}>
          ...
          <div className="mb-5.5">
            ...
            <select
              ...
              onChange={(e: ChangeEvent<HTMLSelectElement>) => {
                ...
                setSelectedSubCommittee("");
                setLevels([]);
                setSelectedPositions(new Set()); // Clear positions on committee change
              }}
              required
              ...
              {positions.map((position) => (
                <div key={position.positionId} className="flex items-center">
                  <input
                    ...
                    onChange={() => handlePositionChange(position.positionId)}
                    className="mr-2"
                  />
                  <label
          ...
          <div className="flex gap-4">
            <button
              type="submit"
              className="hover:bg-primary-dark rounded bg-primary px-6 py-2.5 text-sm font-medium text-white shadow-md"
              disabled={isFormDisabled}
            >
              सुरक्षित गर्नुहोस्
            </button>
            <button
              ...
              className="bg-gray-200 hover:bg-gray-300 rounded px-6 py-2.5 text-sm font-medium text-black shadow-md"
            >
              रद्द गर्नुहोस्
            </button>
          </div>
