...
const SocialLinkDisplayer: React.FC = () => {
  ...
  if (loading) {
    return (
      <div className="w-full">
        <p className="text-center py-4 text-gray-600 dark:text-gray-400">Loading social links...</p>
      </div>
    );
  }
  ...
  const latestLinks = socialLinks
    ...
    .sort(compareDates)
    .slice(0, 5); // Get the first 5 entries (most recent)
  ...
  return (
    <div className="w-full mt-4">
      <div className="bg-white dark:bg-boxdark rounded-lg shadow-sm border border-stroke dark:border-strokedark">
        <div className="p-3 sm:p-4 border-b border-stroke dark:border-strokedark">
            ...
            <div className="space-y-3">
              {latestLinks.map((socialLink) => {
                ...
                return (
                  <div
                    key={socialLink.id}
                    className="group p-3 sm:p-4 rounded-lg border border-gray-200 dark:border-gray-700 hover:border-primary/30 hover:shadow-md transition-all duration-200 bg-white dark:bg-gray-800"
                  >
                    <div className="flex items-start space-x-2 sm:space-x-3">
                      <div className="flex-shrink-0">
                        <div className={`w-8 h-8 sm:w-10 sm:h-10 ${socialIcon.bgColor} rounded-lg flex items-center justify-center`}>
                          <IconComponent className={`w-4 h-4 sm:w-5 sm:h-5 ${socialIcon.color}`} />
                        </div>
                      </div>
                      ...
                      <div className="flex-1 min-w-0 pr-1 sm:pr-2">
                        <div className="space-y-1 sm:space-y-2">
                          <a
                            ...
                            className="text-sm sm:text-base font-medium text-gray-900 dark:text-white hover:text-primary transition-colors line-clamp-1 block"
                          >
                            {socialLink.linkName}
                          </a>
                            ...
                            <div className="flex flex-col space-y-1 sm:flex-row sm:items-center sm:space-x-3 sm:space-y-0 text-xs text-gray-500 dark:text-gray-400">
                              {socialLink.linkPublisher && (
                                <span className="flex items-center">
                                  ...
                                  <span className="truncate max-w-[120px] sm:max-w-none">{socialLink.linkPublisher}</span>
                                </span>
                              )}
                              {socialLink.linkDate && (
                                <span className="flex items-center">
